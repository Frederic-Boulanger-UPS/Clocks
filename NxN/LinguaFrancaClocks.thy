...
text \<open>
Time is represented as a pair of natural numbers. The first item of the pair is the logical time, 
the second item is a causality index used to model super-dense time, in which several events may
occur at the same logical time, but still be ordered in a succession.

A clock represents an event that may occur or not at any time. An event may occur several times 
at the same logical time, but at different indices.

We model a clock as a function from nat\<times>nat to bool, which is True at every
instant when the clock ticks (the event occurs). 
\<close>

type_synonym clock = \<open>(nat\<times>nat) \<Rightarrow> bool\<close>

text \<open>
An event @{term \<open>c\<close>} occurs at logical time @{term \<open>t\<close>} if there is an index @{term \<open>k\<close>}
for which @{term \<open>c\<close>} ticks at @{term \<open>(t, k)\<close>}.
\<close>
definition occurs :: \<open>[clock, nat] \<Rightarrow> bool\<close>
  where \<open>occurs c t \<equiv> \<exists>k. c (t, k)\<close>

text \<open>
A Zeno clock has an infinite number of ticks at a logical instant.
Logical time stops at the smallest instant when this occurs.
\<close>
definition zeno :: \<open>clock \<Rightarrow> bool\<close>
  where \<open>zeno c \<equiv> \<exists>n. \<not>finite {k. c (n, k)}\<close>

subsection \<open>Periodic clocks\<close>
text \<open>
A clock is (k,p)-periodic if it ticks at instants separated by p units of logical time,
starting at logical time k.
\<close>
definition kp_periodic :: \<open>[nat, nat, clock] \<Rightarrow> bool\<close>
  where \<open>kp_periodic k p c \<equiv>
    (p > 0) \<and> (\<forall>n. occurs c n = ((n \<ge> k) \<and> ((n - k) mod p = 0)))\<close>
...
lemma one_periodic_ticks:
  assumes \<open>kp_periodic k 1 c\<close>
    ...
    shows \<open>occurs c n\<close>
using assms kp_periodic_def occurs_def by simp
...
definition p_sporadic :: \<open>[nat, clock] \<Rightarrow> bool\<close>
  where \<open>p_sporadic p c \<equiv> (\<forall>t. occurs c t \<longrightarrow> (\<forall>t'. (t < t' \<and> t' \<le> t+p) \<longrightarrow> \<not>(occurs c t')))\<close>
...
proof -
  define c :: clock where \<open>c = (\<lambda>t. (fst t) mod 2 = 0)\<close>
  define c' :: clock where \<open>c' = (\<lambda>t. (fst t) \<ge> 1 \<and> ((fst t)-1) mod 2 = 0)\<close>

  have \<open>p_periodic 2 c\<close> unfolding p_periodic_def kp_periodic_def occurs_def
                        using c_def by auto
  ...
  proof -
    { assume \<open>sporadic (c \<oplus> c')\<close>
      from this obtain p where *:\<open>p > 0\<close> and \<open>p_sporadic p (c \<oplus> c')\<close>
        using sporadic_def by blast
      hence \<open>\<forall>t. occurs (c\<oplus>c') t \<longrightarrow> (\<forall>t'. (t < t' \<and> t' \<le> t+p) \<longrightarrow> \<not>(occurs (c\<oplus>c') t'))\<close>
        by (simp add:p_sporadic_def)
      moreover have \<open>occurs (c\<oplus>c') 0\<close> using c_def c'_def merge_def occurs_def by simp
      moreover have \<open>occurs (c\<oplus>c') 1\<close> using c_def c'_def merge_def occurs_def by simp
...
definition ticks_up_to :: \<open>[clock, nat] \<Rightarrow> nat\<close>
  where \<open>ticks_up_to c n = card {(t, k). t \<le> n \<and> c (t, k)}\<close>

lemma ticks_up_to_mono:
  assumes \<open>\<not>zeno c\<close>
    shows \<open>mono (ticks_up_to c)\<close>
proof -
  from assms zeno_def have *:\<open>\<forall>t. finite {k. c (t, k)}\<close> by simp
  have \<open>\<forall>n. {(t, k). t \<le> n \<and> c (t, k)} = (\<Union>t \<le> n. {(t, k). c (t, k)})\<close> oops
  hence \<open>\<forall>n. finite {(t, k). t \<le> n \<and> c (t, k)}\<close> 
  { fix n n' :: nat
    assume \<open>n \<le> n'\<close>
    hence \<open>{(t, k). t \<le> n \<and> c (t, k)} \<subseteq> {(t, k). t \<le> n' \<and> c (t, k)}\<close> by auto
    with * have  \<open>card {(t, k). t \<le> n \<and> c (t, k)} \<le> card {(t, k). t \<le> n' \<and> c (t, k)}\<close>
      sledgehammer
(* No longer true with superdense time *)
(*
...
*)
(* Cannot use this with superdense time where the number of ticks at a logical instant is not bounded. *)
(*
...
definition tick_count ::\<open>[clock, nat, nat] \<Rightarrow> nat\<close>
  where \<open>tick_count c t\<^sub>0 n \<equiv> card {(t, k). t\<^sub>0 \<le> t \<and> t < t\<^sub>0+n \<and> c (t, k)}\<close>
